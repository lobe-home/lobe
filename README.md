...
**Everything is off by default** — nothing changes any page until you enable it in
the popup. The popup lists every enhancement, shows which apply to the current page,
and lets you toggle each on or off (or use **Enable all / Disable all**).
...
- See the **full** list of enhancements, grouped into category tabs, on **any**
  page, even an unrelated one. Ones that apply to the current page are tagged
  "On this page"; the rest say "Other pages".
...
- Toggle any enhancement on or off, or use **Enable all / Disable all** to flip
  everything at once. Your choices are saved in `chrome.storage.sync`, so they
  follow you across signed-in browsers.
...
- **`css`** : a string (or `url => string`) injected as a `<style>` tag while the
  enhancement is active, and removed when it's turned off. A rule with
  `!important` overrides the platform's own inline styles. Prefer this whenever
  styling can do the job, it's declarative and cleanly reversible.
...
`manifest.json` matches `*.outsystemscloud.com`, `*.outsystemsenterprise.com`, and
`www.outsystems.com/forums`. If your OutSystems environment is on a **self-hosted /
custom domain**, or you're adding an enhancement for another OutSystems site, add
the host to `content_scripts[0].matches` in `manifest.json`.
...
No network access, no analytics, no data leaves your browser.
